  ...
  return (
    <div className="space-y-3 rounded border bg-card p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <h2 className="text-lg font-semibold">Agent 설치 파일</h2>
          <p className="text-xs text-muted-foreground">
            generic 인스톨러 한 개를 모든 PC에 그대로 배포할 수 있습니다. 더블클릭만으로
            deviceId 자동 생성 → 등록까지 끝납니다 (open enrollment).
          </p>
          <p className="text-xs text-muted-foreground">
            서버에서 <code className="font-mono">STREAM_AGENT_REQUIRE_TOKEN=true</code>를 설정한 경우에만
            아래 “1회용 토큰”을 사용하세요.
          </p>
        </div>
        <a
      ...
      <details className="rounded border border-dashed p-3">
        <summary className="cursor-pointer text-sm font-medium">
          고급: 1회용 토큰 발급 (PC 1대당 1회 사용)
        </summary>
        <div className="mt-3 space-y-3">
          <div className="flex items-center justify-between">
            <p className="text-xs text-muted-foreground">
              지정한 별칭/소유자로 1회용 토큰을 발급하고, 토큰이 박힌 파일명으로 인스톨러를 받습니다.
            </p>
            <Button onClick={() => setOpen((v) => !v)} variant={open ? "outline" : "default"} size="sm">
              {open ? "닫기" : "+ 토큰 발급"}
            </Button>
          </div>
